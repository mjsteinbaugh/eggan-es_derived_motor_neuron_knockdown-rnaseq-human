...
params:
    bcb_file: "data/2018-02-02/bcb.rda"
    output_dir: "."
---
...
```{r copy_to_dropbox, message=TRUE}
dropbox_counts_list <- copyToDropbox(
    c(
        raw_counts = file.path(results_dir, "raw_counts.csv.gz"),
        normalized_counts = file.path(results_dir, "normalized_counts.csv.gz"),
        tpm = file.path(results_dir, "tpm.csv.gz")
    ),
    dir = basename(getwd())
)
...
```{r file_downloads}
file_downloads <- vapply(
    X = dropbox_counts_list,
    FUN = function(x) {
        url <- x[["url"]]
        url <- gsub("\\?dl=0$", "", url)
        name <- x[["name"]]
